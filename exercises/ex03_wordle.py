...
def contains_char(word: str, char: str) -> bool:
    """returns a true or false based on whether the current character is present in the user's guess"""
    assert len(char) == 1
    chr_exists = False
...
def emojified(user_guess: str, secret_word: str) -> str:
    """makes a string of emojis based on whether the guesses are ight, wrong or misplaced"""
    # define colors
...
def input_guess(expected_len: int) -> str:
    """function that checks the length of the user's guess with the length of the secret word"""
...
def main() -> None:
    """main function that runs the program, has the rest of the functions nested within it"""
    turn = 1
    secret = "codes"
    win = False
